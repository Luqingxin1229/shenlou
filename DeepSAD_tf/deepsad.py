...
from networks.multiCNN import multi_CNN, multi_CNN_Decoder
from networks.multiCNN_base import multi_CNN_Decoder_base, multi_CNN_base
...
class DeepSAD():
    ...
    def build_model(self):
        ...
        if self.model_name == 'multi_CNN_0':
            self.model = multi_CNN_0()
        if self.model_name == 'multi_CNN':
            self.model = multi_CNN()
        if self.model_name == 'mnist_LeNet':
            self.model = MNIST_LeNet()
        if self.model_name == 'cifar10_LeNet':
            self.model = CIFAR10_LeNet()
        if self.model_name == 'multi_CNN_base':
            self.model = multi_CNN_base()
        if self.model_name == 'CNN_0':
            self.model = CNN_0()
    ...
    def build_decoder(self, net_name):
        ...
        '''
        if self.model_name == 'multi_CNN_0':
            self.decoder = multi_CNN_Decoder_0()
        if self.model_name == 'multi_CNN':
            self.decoder = multi_CNN_Decoder()
        if self.model_name == 'mnist_LeNet':
            self.decoder = MNIST_LeNet_decoder()
        if self.model_name == 'cifar10_LeNet':
            self.decoder = CIFAR10_LeNet_decoder()
        if self.model_name == 'multi_CNN_base':
            self.decoder = multi_CNN_Decoder_base()
        if self.model_name == 'CNN_0':
    ...
    def save_model(self, model_name):
        ...
        saved_c = self.c
        saved_c = saved_c.numpy()
        np.save("saved_models/" + model_name[-7:] + "c.npy", saved_c)