...
class multi_CNN(Model):
    def __init__(self, req_dim=32):
        ...
        self.conv1_1 = tf.keras.Sequential([
            layers.Conv2D(64, (3, 1), strides=(1, 1), padding='same', input_shape=input_shape),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
        self.conv1_2 = tf.keras.Sequential([
            layers.Conv2D(128, (3, 1), strides=(1, 1), padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
        ...
        self.conv2_1 = tf.keras.Sequential([
            layers.Conv2D(64, (5, 1), strides=(1, 1), padding='same', input_shape=input_shape),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
        self.conv2_2 = tf.keras.Sequential([
            layers.Conv2D(128, (5, 1), strides=(1, 1), padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
        ...
        self.conv3_1 = tf.keras.Sequential([
            layers.Conv2D(64, (7, 1), strides=(1, 1), padding='same', input_shape=input_shape),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
        self.conv3_2 = tf.keras.Sequential([
            layers.Conv2D(128, (7, 1), strides=(1, 1), padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),
            layers.MaxPooling2D()
        ])
...
class multi_CNN_Decoder(Model):
    def __init__(self, req_dim=32):
        ...
        self.deconv1_1 = tf.keras.Sequential([
            ...
            layers.Conv2DTranspose(128, (3, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),

            layers.Conv2DTranspose(64, (3, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2)
        ])
        self.deconv1_2 = tf.keras.Sequential([
            layers.Reshape((5, 25, 128)),
            layers.Conv2DTranspose(128, (5, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),

            layers.Conv2DTranspose(64, (5, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2)
        ])
        self.deconv1_3 = tf.keras.Sequential([
            layers.Reshape((5, 25, 128)),
            layers.Conv2DTranspose(128, (7, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2),

            layers.Conv2DTranspose(64, (7, 1), strides=2, padding='same'),
            layers.BatchNormalization(epsilon=1e-4),
            layers.LeakyReLU(alpha=0.2)
        ])
        self.concatenate = layers.Concatenate(axis=-1)
