...
if __name__ == '__main__':
    x_test, y_test= audio_data_to_net(excel_dir='air_test.csv', target_samples=50_000, target_sr=10_000,
                               root_dir='UrbanSound8k/UrbanSound8K/audio')
    # print(y_test)
    # jiance(x_test, 64)
    svdd = tf.saved_model.load('deep_svdd_saved_model')
    ...
    for i in range(62):
        ...
        score = svdd.predict_fn(a)
        print(score, y_test[i])
        # 精度计算
        if score >=0.3:
            y_pred[i] = 1
        else:y_pred[i] = 0
